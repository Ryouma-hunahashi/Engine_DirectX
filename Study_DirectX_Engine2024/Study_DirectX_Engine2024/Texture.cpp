...
HRESULT Texture::Create(const char* fileName)
{
	...
	// シェーダリソース生成
	hr = CreateShaderResourceView(DirectXManager::GetInstance().GetDevice(), image.GetImages(), image.GetImageCount(), mdata, &m_pSRV);
	if (SUCCEEDED(hr))
	{
...
HRESULT Texture::CreateResource(D3D11_TEXTURE2D_DESC& desc, const void* pData)
{
	...
	hr = DirectXManager::GetInstance().GetDevice()->CreateTexture2D(&desc, pData ? &data : nullptr, &m_pTex);
	...
	// 生成
	hr = DirectXManager::GetInstance().GetDevice()->CreateShaderResourceView(m_pTex, &srvDesc, &m_pSRV);
	if (SUCCEEDED(hr))
	{
		m_width = desc.Width;
...
{
	DirectXManager::GetInstance().GetContext()->ClearRenderTargetView(m_pRTV, color);
}
...
HRESULT RenderTarget::CreateFromScreen()
{
	...
	hr = DirectXManager::GetInstance().GetSwapChain()->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&m_pTex);
	if (FAILED(hr)) { return hr; }
	...
	hr = DirectXManager::GetInstance().GetDevice()->CreateRenderTargetView(m_pTex, &rtvDesc, &m_pRTV);
	if (SUCCEEDED(hr))
	{
		D3D11_TEXTURE2D_DESC desc;
		m_pTex->GetDesc(&desc);
...
HRESULT RenderTarget::CreateResource(D3D11_TEXTURE2D_DESC& desc, const void* pData)
{
	...
	// 生成
	return DirectXManager::GetInstance().GetDevice()->CreateRenderTargetView(m_pTex, &rtvDesc, &m_pRTV);
}
...
{
	DirectXManager::GetInstance().GetContext()->ClearDepthStencilView(m_pDSV, D3D11_CLEAR_DEPTH | D3D11_CLEAR_STENCIL, 1.0f, 0);
}
...
HRESULT DepthStencil::CreateResource(D3D11_TEXTURE2D_DESC& desc, const void* pData)
{
	...
	// 生成
	return DirectXManager::GetInstance().GetDevice()->CreateDepthStencilView(m_pTex, &dsvDesc, &m_pDSV);
}