...
void Shader::WriteBuffer(UINT slot, void* pData)
{
	if (slot < m_pBuffers.size())
		DirectXManager::GetInstance().GetContext()->UpdateSubresource(m_pBuffers[slot], 0, nullptr, pData, 0, 0);
}
void Shader::SetTexture(UINT slot, Texture* tex)
{
	...
	switch (m_kind)
	{
	case Vertex:	DirectXManager::GetInstance().GetContext()->VSSetShaderResources(slot, 1, &pTex); break;
	case Pixel:		DirectXManager::GetInstance().GetContext()->PSSetShaderResources(slot, 1, &pTex); break;
	case Hull:		DirectXManager::GetInstance().GetContext()->HSSetShaderResources(slot, 1, &pTex); break;
	case Domain:	DirectXManager::GetInstance().GetContext()->DSSetShaderResources(slot, 1, &pTex); break;
	}
}
...
{
	return DirectXManager::GetInstance().GetDevice()->CreatePixelShader(pData, size, NULL, &m_pPS);
}
...
{
	DirectXManager::GetInstance().GetContext()->HSSetShader(nullptr, nullptr, 0);
}
HRESULT HullShader::MakeShader(void* pData, UINT size)
{
	return DirectXManager::GetInstance().GetDevice()->CreateHullShader(pData, size, NULL, &m_pHS);
}
...
{
	DirectXManager::GetInstance().GetContext()->DSSetShader(nullptr, nullptr, 0);
}
HRESULT DomainShader::MakeShader(void* pData, UINT size)
{
	return DirectXManager::GetInstance().GetDevice()->CreateDomainShader(pData, size, NULL, &m_pDS);
}